...
| Relationship-memory library and detail | `Features/RelationshipMemory/RelationshipMemoryVaultView.swift`: `RelationshipMemoryVaultView` and its library item/row presentation; `Features/RelationshipMemory/RelationshipMemoryDetailView.swift`: `RelationshipMemoryDetailView`, `RelationshipVoiceCardDetailView`, and their feature-private detail chrome and notice; `Features/RelationshipMemory/RelationshipMemoryPersistence.swift`: `performConfirmedMemoryDeletion` and `performRelationshipMemorySave`; shared empty/detail presentation is in `Components/`. The composer's own memory controls remain in `MomentExperienceView.swift` |
