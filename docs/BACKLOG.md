...
| `P1-53` | Critical smoke AI service gate | `scripts/test_critical_smoke.sh` includes stable AI service coverage (`test/services/ai_service_test.dart` or an equivalent deterministic subset), the script fails on AI parsing/error-classification regressions, and `docs/DEVOPS.md` reflects the blocking smoke composition. |
