...
| `P1-09` | Release key scan guard | CI/release includes a blocking secret scan gate (for example gitleaks/trufflehog) for pull requests and release candidates, scan output is archived as evidence, and remediation guidance covers key rotation plus history rewrite workflow when leakage is detected. |
