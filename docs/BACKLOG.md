...
| `P1-43` | Typography and contrast token enforcement | All core screens (`auth`, `home`, `generate`, `results`, `settings`, `paywall`, `calendar`, `history`) use theme text styles (`Theme.of(context).textTheme`) and `AppColors`/`ColorScheme` tokens instead of ad-hoc `TextStyle` and `Colors.grey/white/black` literals for user-facing text/surfaces. A CI lint/check fails on new direct literal color usage in `lib/features/**` (with documented exceptions for brand-required controls), and updated critical goldens confirm no light-on-light regressions. |
